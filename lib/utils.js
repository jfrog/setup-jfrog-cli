...
class Utils {
    ...
    static constructJobSummary() {
        return __awaiter(this, void 0, void 0, function* () {
            const homedir = Utils.getJobsTempDirectoryPath();
            let fileContent = '';
            let isAnySectionFound = false;
            // Read each section
            for (const section of Utils.JOB_SUMMARY_SECTIONS) {
                try {
                    const content = yield fs_1.promises.readFile(path.join(homedir, section + '.md'), 'utf-8');
                    if (content.trim() !== '') {
    ...
    wrapContentInSection(sectionTitle, markdown) {
    ...
    static getInitialContent() {
        ...
        let mainTitle = `<p><h1><picture><img src="${imgSrc}" style="margin: 0 0 -10px 0" width="65px" alt="JFrog logo"></picture> JFrog Job Summary</h1></p>`;
        return mainTitle + '\n\n' + packagesLink + '\n\n';
    }
    static getJobSummaryEnvVars() {
        let projectKey = process.env.JFROG_CLI_BUILD_PROJECT;
    ...
    static getJobsTempDirectoryPath() {
        const homedir = process.env.JFROG_CLI_JOB_SUMMARY_HOME_DIR;
        if (!homedir) {
...
Utils.JOB_SUMMARY_DIR_PATH = 'jfrog-job-summary';
// Job summary section files
Utils.JOB_SUMMARY_SECTIONS = ['upload-data', 'build-publish', 'security'];
// Inputs
// Version input
