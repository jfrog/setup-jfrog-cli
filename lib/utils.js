...
class Utils {
    ...
    static setOidcTokenID(jfrogCredentials) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                jfrogCredentials.oidcTokenId = yield core.getIDToken('jfrog-github');
            }
            catch (error) {
    ...
    static setCliEnv() {
        ...
        let ciJobID = process.env.GITHUB_WORKFLOW;
        if (ciJobID) {
            Utils.exportVariableIfNotSet('JFROG_CLI_CI_JOB_ID', ciJobID);
        }
        let ciRunID = process.env.GITHUB_RUN_ID;
        if (ciRunID) {
            Utils.exportVariableIfNotSet('JFROG_CLI_CI_RUN_ID', ciRunID);
        }
        // Set the GitHub repository name or default to an empty string.
        core.exportVariable('JFROG_CLI_SOURCECODE_REPOSITORY', (_a = process.env.GITHUB_REPOSITORY) !== null && _a !== void 0 ? _a : '');
...
Utils.OIDC_CLI_TOKEN_ID_ENV_VAR_NAME = 'JFROG_CLI_OIDC_EXCHANGE_TOKEN_ID';
// Application Key enn var name used by the CLI
Utils.APPLICATION_KEY_CLI_ENV_VAR_NAME = 'JFROG_CLI_APPLICATION_KEY';
