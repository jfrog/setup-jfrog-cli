    ...
    static getJfrogAccessTokenThroughOidcProtocol(jfrogCredentials, jsonWebToken, oidcProviderName) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (jfrogCredentials.accessToken) {
                ...
                let tokenParts = jfrogCredentials.accessToken.split('.');
                ...
                let payload = JSON.parse(Buffer.from(tokenParts[1], 'base64').toString('utf8'));
                if (!payload || !payload.sub) {
                    throw new Error(`OIDC invalid access token format`);
                }
                core.info('token payload=' + payload);
                if (payload.sub.startsWith('jfrt@') || payload.sub.includes('/users/')) {
                    let lastSlashIndex = payload.sub.lastIndexOf('/');
                    let lastSubstring = payload.sub.substring(lastSlashIndex + 1);
