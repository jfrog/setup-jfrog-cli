...
exports.Section = exports.Utils = void 0;
...
class Utils {
    ...
    static extractDownloadDetails(repository, jfrogCredentials) {
        ...
        let serverObj = Utils.getServerObjFromConfig();
    ...
    /**
     * Generates GitHub workflow Summary
    ...
    static readCLIMarkdownSections() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const sectionName of Utils.JOB_SUMMARY_MARKDOWN_SECTIONS_NAMES) {
                ...
                fileContent += yield Utils.readSummarySection(fullPath, sectionName);
            }
     ...
     */
    static getJobSummaryVars() {
        let serverObj = this.getServerObjFromConfig();
        let projectKey = this.getProjectKey(serverObj);
        let platformUrl = this.getPlatformUrl(serverObj);
        let packagesUrl = this.constructPackagesUrl(platformUrl, projectKey);
        return [packagesUrl, projectKey];
    }
    static getProjectKey(serverObj) {
        let projectKey = process.env.JF_PROJECT;
        if (!projectKey) {
    ...
    static getPlatformUrl(serverObj) {
    ...
    static constructPackagesUrl(platformUrl, projectKey) {
...
Utils.JOB_SUMMARY_MARKDOWN_SECTIONS_NAMES = ['upload', 'build-info', 'security'];
// Inputs
// Version input
