...
class Utils {
    ...
    static getSeparateEnvConfigCommand() {
        /**
    ...
    static configJFrogServers() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (let configToken of Utils.getConfigTokens()) {
                let importCmd = useOldConfig ? ['rt', 'c', 'import', configToken] : ['c', 'import', configToken];
                yield Utils.runCli(importCmd);
            }
            let configCommand = Utils.getSeparateEnvConfigCommand();
            if (configCommand) {
