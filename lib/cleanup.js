...
function checkBuildInfoExistsInArtifactory() {
    return __awaiter(this, void 0, void 0, function* () {
        const buildName = process.env.JFROG_CLI_BUILD_NAME;
        const buildNumber = process.env.JFROG_CLI_BUILD_NUMBER;
        if (!buildName || !buildNumber) {
            core.warning('Build name or number is not defined.');
            return;
        }
        // Define the API endpoint for the build-info
        const url = `http://localhost:8081/artifactory/api/build/${buildName}/${buildNumber}`;
        try {
            // Send GET request to the API
            const response = yield new http_client_1.HttpClient().get(url);
