...
function addCachedJfToPath() {
    let version = core.getInput(utils_1.Utils.CLI_VERSION_ARG);
    if (version == utils_1.Utils.LATEST_CLI_VERSION) {
        // If the version is 'latest', we keep it on cache as 100.100.100
        version = utils_1.Utils.LATEST_SEMVER;
    }
    let jfFileName = utils_1.Utils.getJfExecutableName();
    let jfrogFileName = utils_1.Utils.getJFrogExecutableName();
    return utils_1.Utils.loadFromCache(jfFileName, jfrogFileName, version);
}
