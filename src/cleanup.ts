...
async function buildInfoPostTasks() {
    ...
    const disableJobSummary: boolean = core.getBooleanInput(Utils.JOB_SUMMARY_DISABLE) || !JobSummaryUtils.isJobSummarySupported();
...
async function hasUnpublishedModules(workingDirectory: string): Promise<boolean> {
    ...
    const origValue: string | undefined = process.env[JobSummaryUtils.JFROG_CLI_COMMAND_SUMMARY_OUTPUT_DIR_ENV];
    try {
        core.startGroup('Check for unpublished modules');
        // Avoid saving a command summary for this dry-run command
        core.exportVariable(JobSummaryUtils.JFROG_CLI_COMMAND_SUMMARY_OUTPUT_DIR_ENV, '');
        ...
        core.exportVariable(JobSummaryUtils.JFROG_CLI_COMMAND_SUMMARY_OUTPUT_DIR_ENV, origValue);
        core.endGroup();
    }
}
...
async function generateJobSummary() {
    try {
        ...
        await JobSummaryUtils.setMarkdownAsJobSummary();
        await JobSummaryUtils.populateCodeScanningTab();
        // Clear files
        await JobSummaryUtils.clearCommandSummaryDir();
    } catch (error) {
