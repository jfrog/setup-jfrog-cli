...
export class Utils {
    ...
    private static readonly FROGBOT_CONFIG_DIR_NAME: string = '.frogbot';
    // Config file name
    private static readonly FROGBOT_CONFIG_FILE: string = 'frogbot-config.yml';
    ...
    /**
     * Retrieves the application key from input or frogbot configuration file.
     *
     * This method attempts to retrieve the application key from the GitHub Action input.
     * If the input is not provided, it reads frogbot configuration file from the file system.
    ...
    private static async getApplicationKey(): Promise<string> {
        ...
        const configFilePath: string = path.join(this.FROGBOT_CONFIG_DIR_NAME, this.FROGBOT_CONFIG_FILE);
        try {
            ...
            const applicationKey: string = configObj['application-key'];
            if (!applicationKey) {
    ...
    /**
     * Reads frogbot configuration file from file system.
     *
     * This method attempts to read frogbot configuration file from the specified relative path.
    ...
    private static async readConfigFromFileSystem(configRelativePath: string): Promise<string> {
        ...
        if (!existsSync(configRelativePath)) {
            core.debug(`frogbot-config.yml not found in ${configRelativePath}`);
            return '';
        }

        core.debug(`frogbot-config.yml found in ${configRelativePath}`);
